...
class BackupService {
  ...
  Future<BackupData> createBackup() async {
    try {
      ...
      final allPrescriptions = await database.prescriptionsDao.watchAllPrescriptions().first;
      ...
      for (final user in allUsers) {
        final settings = await database.userSettingsDao.getSettingsForUser(user.id);
        if (settings != null) {
          userSettings.add(settings);
        }
        
        final events = await database.doseEventsDao.watchAllDoseEvents(user.id).first;
      ...
      final backupData = BackupData(
        ...
        users: allUsers.map((user) => UserBackup(
          id: user.id,
          name: user.name,
          createdAt: user.createdAt,
        )).toList(),
        userSettings: userSettings.map((settings) => UserSettingBackup(
        ...
        prescriptions: allPrescriptions.map((prescription) => PrescriptionBackup(
          id: prescription.id,
        ...
        doseEvents: doseEvents.map((event) => DoseEventBackup(
  ...
  try {
    ...
    if (Platform.isAndroid) {
      ...
      if (!status.isGranted) {
        print('⚠️  Permissão de armazenamento não concedida, tentando continuar...');
      }
    }
  ...
  Future<void> shareBackup() async {
    try {
      ...
      await Share.shareXFiles(
        ...
        text: 'Backup completo do CheckPills - App de gestão de medicamentos\n\nData: ${DateTime.now().toString()}',
      );
  ...
  try {
    ...
    for (final file in files) {
      if (file is File && file.path.endsWith('.json') && file.path.contains('checkpills_backup_')) {
...
Future<void> deleteBackup(String filePath) async {
  ...
  await database.transaction(() async {
    ...
    print('✅ ${backupData.users.length} usuários restaurados');
    ...
    print('✅ ${backupData.userSettings.length} configurações restauradas');

    // 4. Restaurar prescrições
    print('💊 Restaurando prescrições...');
    for (final prescriptionBackup in backupData.prescriptions) {
      await database.prescriptionsDao.addPrescription(
        PrescriptionsCompanion(
          ...
          enableNotifications: Value(prescriptionBackup.enableNotifications),
          notifyMinutesBefore: Value(prescriptionBackup.notifyMinutesBefore),
    ...
    print('✅ ${backupData.prescriptions.length} prescrições restauradas');

    // 5. Restaurar eventos de dose
    print('📅 Restaurando eventos de dose...');
    ...
    print('✅ ${backupData.doseEvents.length} eventos de dose restaurados');
  });
  ...
  Future<bool> verifyBackupIntegrity(BackupData backupData) async {
    try {
      ...
      final usersWithSettings = backupData.userSettings.map((s) => s.userId).toSet();
      ...
      final validPrescriptionIds = backupData.prescriptions.map((p) => p.id).toSet();
  ...
  try {
    ...
    final backupData = BackupData.fromJson(jsonDecode(jsonString));
    print('✅ Backup carregado: ${backupData.users.length} usuários, ${backupData.prescriptions.length} prescrições');
